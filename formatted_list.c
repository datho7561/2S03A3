// PLEASE NOTE:
//  This is a copy of list.c that has been run through
//  clang-format
...
static HBnodePtr generateHBlist(int n, int m, int a[]) {
  ...
  int slLength = (int)((m + 1) * ((double)rand() / RAND_MAX)),
      val_arr[slLength], i;
  ...
  // * (500 - *a)               --> Scale from somewhere between [0,0] or
  // [0,500),
  //                                depending on how big *a (the start of the
  //                                list) is The scaling is done this way so
  //                                that no matter what, the number is <=
  //                                RAND_MAX. This is to avoid integer overflow.
  // + *a                       --> Add the value of the start of the list so
  // that
